...
Proof.
  ...
  - intro g.
    ...
    + intro h.
      transport_paths FlFr.
      rewrite ClassifyingSpace_rec_beta_bloop, <- 2 bloop_pp.
      apply ap; symmetry; apply comm.
  - intros; revert bh.
    srapply ClassifyingSpace_ind_hprop; simpl.
    rewrite <- bloop_pp.
    apply ap.
    rapply issemigrouppreserving_grp_homo.
Defined.

Definition fmap11_B'_bbase {G H K : Group}
  (f1 : G $-> K) (f2 : H $-> K)
  (comm: forall g h, f1 g * f2 h = f2 h * f1 g)
...
Definition fmap11_B'_bbase' `{funext : Funext} {G H K : Group}
  (f1 : G $-> K) (f2 : H $-> K)
  (comm: forall g h, f1 g * f2 h = f2 h * f1 g)
