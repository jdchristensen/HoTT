...
Definition issigmacompact_issearchable {A : Type}
  : IsSearchable A -> IsSigmaCompact A.
Proof.
  intros h P dP.
  destruct (h (fun x => ~ (P x)) _) as [w hw].
  destruct (dP w) as [x|y].
  - exact (inl (w; x)).
...
Proof.
  ...
  - intro c.
    ...
    + exact (inl r).
  - intros [l|r].
    + exact ((fst (iff_iscompact_issigmacompact A)) (iscompact_empty' l)).
...
(** Assuming univalence, if the domain of a surjective map is searchable or compact, then so is its codomain. *)
...
Proof.
  ...
  destruct ((fst issigmacompact_iff_not_or_issearchable) c) as [n|s].
