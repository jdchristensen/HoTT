...
Section DescentGQ.
  ...
  Section DependentDescentWithFamily.
    ...
    Definition c_descentfam_sect : forall x : Coeq f g, forall px : c_bundle_descent x, Q x px
      := Coeq_ind _ f_A cglue_descentfam_sect.

    Definition c_descentfam_sect_beta (b : B) (p : P_A (f b))
      : c_descentfam_sect (coeq (g b)) (e_P b p) = c_descentfam_e b (c_descentfam_sect (coeq (f b)) p)
      := (e_f b p)^.

    Definition c_descentfam_sect_beta_ap (b : B)
