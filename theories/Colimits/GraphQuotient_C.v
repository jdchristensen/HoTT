...
(** By univalence, we can alternately define induction by families over the graph, rather than families over the graph quotient. *)
...
Section Descent.
  ...
  Global Arguments gqd_fam {_ _} _.
  Global Arguments gqd_e {_ _} _ {_ _}.

  (** Let [A] be a graph, with edges indexed by [R].  *)
  Context {A : Type} {R : A -> A -> Type}.

  (** Descent data defines a type family over [GraphQuotient R]. *)
  ...
  Global Arguments gqds_e {_ _} _ {_ _} _ _.

  (** Transporting [gqds_sect f a] over [Q] along [gqglue r] is evaluation at the other endpoint [b] of the edge [r]. *)
  ...
  (** Transporting [gqdcs_sect f a] over [Q] along [gqglue r] is evaluation at the other endpoint [b] of the edge [r]. *)
...
Section Flattening.
  ...
  Lemma equiv_gqd_flatten : sig (Dgqd Pe) <~> GraphQuotient (fun a b => {r : R a.1 b.1 & gqd_e Pe r a.2 = b.2}).
  Proof.
